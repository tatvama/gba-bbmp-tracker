...
import {
  Binary, Calendar, Languages, Brain, EyeOff, HardDrive, Info, RefreshCw, Save, Check, AlertCircle, Sparkles, ChevronRight
} from "lucide-react";
...
export function ComplaintSettingsForm({ initial }: { initial: ComplaintSettings }) {
  ...
  return (
    <form action={action} className="space-y-6 pb-20">
      {/* Structural Page Header with Breadcrumbs, Title, and Actions */}
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between border-b border-border/40 pb-5">
        <div className="min-w-0 space-y-1.5">
          ...
          <p className="max-w-3xl text-xs sm:text-sm leading-relaxed text-muted-foreground/90 font-medium font-sans">
        ...
        {/* Action buttons at the top right side with correct position */}
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between gap-2">
                <Label className="text-xs font-bold text-slate-800 dark:text-slate-200 flex items-center gap-1.5">
                  ...
                  <button
                    type="button"
                    onClick={() => setActiveHelp(activeHelp === "aiAdvisorPreReminderSlaDays" ? null : "aiAdvisorPreReminderSlaDays")}
                    className="text-slate-400 hover:text-slate-600 transition-colors p-0.5 cursor-pointer"
                    aria-label="Info"
                  >
                    <Info className="h-3.5 w-3.5" />
              ...
              {renderTooltip("aiAdvisorPreReminderSlaDays")}
              <div className="relative rounded-lg overflow-hidden flex shadow-2xs">
                <Input
                  type="number"
                  min={1}
