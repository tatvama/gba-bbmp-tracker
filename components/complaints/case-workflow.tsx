        ...
        {active === "acknowledge" && (
          <StepPanel title="Upload the acknowledgement" hint="Scan or photograph the officer's acknowledgement / “forwarded to the concerned officer” slip. It is OCR'd and AI-summarised.">
            <div className="space-y-4">
              {/* 1. Upload form at the top */}
              <div>
                {ackDocs.length > 0 && <p className="text-xs font-semibold text-muted-foreground mb-2">Upload another document:</p>}
                <ScanCapture
                  complaintId={complaintId}
              ...
              {/* 2. Uploaded documents at the bottom */}
              {ackDocs.length > 0 && (
                <div className="mt-4 border-t pt-4 space-y-3">
                  <p className="text-xs font-semibold text-muted-foreground mb-1">Uploaded documents:</p>
                  {ackDocs.map((doc) => (
        ...
        {active === "reply" && (
          <StepPanel title="Upload the department reply / report" hint="After some days the department replies or files an Action Taken Report. Capture it here; OCR + AI extract the reply and any pending issues.">
            <div className="space-y-4">
              {/* 1. Upload form at the top */}
              <div>
                {replyDocs.length > 0 && <p className="text-xs font-semibold text-muted-foreground mb-2">Upload another reply / report:</p>}
                <ScanCapture
                  complaintId={complaintId}
              ...
              {replyDocs.length > 0 && (
                <div className="mt-4 border-t pt-4 space-y-3">
                  <p className="text-xs font-semibold text-muted-foreground mb-1">Uploaded replies / reports:</p>
                  {replyDocs.map((doc) => (
                    <div key={doc.id} className="rounded-md border bg-muted/30 p-3 animate-fade-in">
                      <div className="mb-2 flex flex-wrap items-center gap-2">
                        <FileText className="h-4 w-4 text-muted-foreground" />
