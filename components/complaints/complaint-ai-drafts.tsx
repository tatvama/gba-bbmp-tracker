...
import { Sparkles, Loader2, Copy, Printer, Save, Check, AlertTriangle, Download, Pencil, Activity } from "lucide-react";
...
export function ComplaintAiDrafts({
  ...
  async function generate() {
    ...
    setLiveText(""); setStageLabel("Connecting to Claude…"); setProgress(5);
    ...
    const poll = async () => {
      ...
      const result = job.result as { text?: string; partial?: boolean; stageLabel?: string; lintWarning?: string | null; truncated?: boolean } | null;
      if (job.status === "done") {
  ...
  return (
    <div className="space-y-4">
      ...
      {!busy && draft && <LetterPreview markdown={draft} />}
