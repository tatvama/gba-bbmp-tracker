...
import { getBBMPWorkByJobNumber } from "@/lib/bbmp-works/search";
...
export default async function BBMPWorkByJobNumberPage({
  params,
}: {
  params: Promise<{ jobNumber: string }>;
}) {
  const { jobNumber } = await params;
  const work = await getBBMPWorkByJobNumber(decodeURIComponent(jobNumber));
  if (!work) notFound();
