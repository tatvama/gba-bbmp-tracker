...
// `cache` on every extractor below: job-audit.ts's document loop calls each of
// these once per matching document (up to DOC_CAP=60) — each extractor's own
// system prompt is identical across every document of that type in the loop.
export async function extractMbBill(ocr: string, cache?: boolean) {
...
export async function extractTimelineDates(ocr: string, docType?: string | null, cache?: boolean) {
...
export async function extractEligibility(ocr: string, cache?: boolean) {
...
export async function extractInsurance(ocr: string, cache?: boolean) {
...
export async function extractRoyalty(ocr: string, cache?: boolean) {
