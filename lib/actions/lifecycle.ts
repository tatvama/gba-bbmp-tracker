 ...
 * require) and the department's latest reply, then AI-analyse what the reply did NOT
 * address — the basis for a counter-reply / escalation.
 */
export async function analyzeReplyGapAction(input: { complaintId: string; replyText?: string }): Promise<ReplyGapResult> {
  ...
  let replyText = (input.replyText ?? "").trim();
  if (!replyText) {
